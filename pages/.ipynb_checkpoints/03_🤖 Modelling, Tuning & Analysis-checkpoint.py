...
if selected_model == model_options_list[1]: 
    ...
    st.caption("""
                ...
                The KNN doesn't create learning model, but it remembers the data points distance (provided) proximity metric.
    ...
    with observations:
        ...
        st.caption("""
                    ...
                    6. Sensitive to noisy data, missing values and outliers
                    
                    7. Optimal value of K — If chosen incorrectly, the model will be under or overfitted to the data
                    
                    8. Need feature scaling, results in additional-step computationally and less interpretable.""".\
                   format(Random_Test_Logloss, round(best_params["LogLoss"],3), round(best_params["Misclassified-Percent"],3)*100))
...
if selected_model == model_options_list[2]:
    ...
    if is_balanced == "Unbalanced":
        ...
        with hyperparameter:
            ...
            with hp_options:
                st.session_state.hp_selected = st.radio(label = "Hyperparameters" ,
                                                         options = ["Alpha","Class Weight","Penalty"], 
                                                         index = 0)
    ...
    with hyperparameter:
        ...
        with hp_details:
            if st.session_state.hp_selected == "C":
                ...
                st.caption("3. A low C makes the decision surface smooth, while a high C aims at classifying all training examples correctly.")
            if st.session_state.hp_selected == "Others":
                st.caption("Base classifier Hyperparameters...")
